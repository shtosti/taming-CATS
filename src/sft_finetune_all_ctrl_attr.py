...
def load_and_prepare_dataset(dataset_name, tokenizer, args):
    ...
    if args.metric_names:
        allowed_metrics = args.metric_names
    else:
        allowed_metrics = [args.metric_name]

    for m in allowed_metrics:
        if m not in metric_mapping:
            raise KeyError(f"Metric '{m}' not found in metric mapping!")
    ...
    def process_instance(row):
        ...
        selected_metric = get_next_metric()
        metric_key_in_dataset = metric_mapping[selected_metric]

        is_source_based = selected_metric in ["FRE", "FKGL", "ARI", "DALE-CHALL"]
        source_metric_value = row["source_metrics"][metric_key_in_dataset] if is_source_based else None
        target_metric_value = row["target_metrics"][metric_key_in_dataset]
        reference_simplification = row["simplification_text"]

        explanation = select_control_token_explanation(control_tokens, selected_metric, target_metric_value) \
            if "explanation" in args.user_prompt_id else None
        
        examples = select_random_control_token_examples(control_tokens, selected_metric) \
...
def parse_args():
    ...
    parser.add_argument("--metric_names", nargs="+", type=str, help="List of ctrl attrs to train on. Overrides --metric_name.")
