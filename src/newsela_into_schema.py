...
def load_newsela():
    ...
    dataset = NewselaDataset(NEWSELA_DIR_PATH, NEWSELA_METADATA_PATH, limit=None)
    dataset.load_data()
    df = dataset.data_df
...
def save_jsonl(dataset, jsonl_data):
    ...
    output_file = f"{output_dir}/{dataset.dataset_name}.jsonl"
    ...
    print(f"Total entries: {len(output_file)}")
