...
def main():
    ...
    prompting_type = "vanilla"  # TODO choose from "vanilla", "reasoning", "transformations"
    ...
    # --- Load dataset ---
    data = load_dataset_from_hf(DATASET, split="validation", slice=3) # specify slice, else defaults to -1 (no slice)
    ...
    output_dir = f"experiments/prompting/{experiment_name}/{prompting_type}/{get_model_short_name(model_name=MODEL)}/{dataset_name}/{run_id}"
    ...
    with open(output_file, "a", encoding="utf-8") as jsonl_file:
        ...
        for item in data:
            if isinstance(item, dict):  # Check if `item` is a dictionary
                input_text = item.get("source_text", "")  # Use .get to avoid errors if "source_text" doesn't exist
            else:
                print(f"Unexpected item format: {item}")
            # Your other code that processes input_text
            ...
            for simplification in item["simplifications"]:
                ...
                if "examples" in user_prompt_id:
                    control_token_examples = select_random_control_token_examples(control_tokens, metric_name)
                ...
                output_data = {
                    ...
                    "model": get_model_short_name(MODEL),
                ...
                print("Model used:", get_model_short_name(MODEL))
                print("Control token used:", f"<{metric_name}={metric_value}>")

                print(f"Generated simplified text: {generated_simplified_text}")
